...
async function initDb() {
  ...
  await run(`CREATE TABLE IF NOT EXISTS interests (
    interest_id INTEGER PRIMARY KEY AUTOINCREMENT,
    interest_name TEXT UNIQUE NOT NULL
  )`);

  await run(`CREATE TABLE IF NOT EXISTS student_interests (
    ...
    PRIMARY KEY (student_profile_id, interest_id)
  )`);

  await run(`CREATE TABLE IF NOT EXISTS project_types (
  ...
  await run(`CREATE TABLE IF NOT EXISTS instructor_supervised_project_types (
    instructor_profile_id INTEGER NOT NULL REFERENCES instructor_profiles(instructor_profile_id),
    project_type_id INTEGER NOT NULL REFERENCES project_types(project_type_id),
    PRIMARY KEY (instructor_profile_id, project_type_id)
  )`);
...
await run(`CREATE TABLE IF NOT EXISTS project_applications (
  ...
  for (const t of ["AI-based Chatbot Systems", "Image Processing Projects", "Web-based AI Tools", "Data Science Projects"]) {
    await run("INSERT OR IGNORE INTO project_types (type_name) VALUES (?)", [t]);
  }

  for (const i of ["Artificial Intelligence", "Machine Learning", "Web Development", "IoT", "Data Science", "Mobile Development"]) {
    await run("INSERT OR IGNORE INTO interests (interest_name) VALUES (?)", [i]);
  }
  ...
  const ik = async (name) => (await get("SELECT interest_id FROM interests WHERE interest_name = ?", [name])).interest_id;
  const tk = async (name) => (await get("SELECT project_type_id FROM project_types WHERE type_name = ?", [name])).project_type_id;
  ...
  for (const iid of [await ik("Artificial Intelligence"), await ik("Machine Learning")]) {
    await run("INSERT OR IGNORE INTO student_interests (student_profile_id, interest_id) VALUES (?, ?)", [sp1.lastID, iid]);
  }
  ...
  for (const tid of [await tk("AI-based Chatbot Systems"), await tk("Image Processing Projects")]) {
    await run("INSERT OR IGNORE INTO instructor_supervised_project_types (instructor_profile_id, project_type_id) VALUES (?, ?)", [ip1.lastID, tid]);
  }
  for (const tid of [await tk("Web-based AI Tools"), await tk("Data Science Projects")]) {
    await run("INSERT OR IGNORE INTO instructor_supervised_project_types (instructor_profile_id, project_type_id) VALUES (?, ?)", [ip2.lastID, tid]);
  }
