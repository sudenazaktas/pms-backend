...
router.post("/:id/apply", authMiddleware, requireRole("student"), async (req, res) => {
  try {
    ...
    const { note } = req.body;
    await run("INSERT INTO project_applications (project_id, student_profile_id, note) VALUES (?, ?, ?)", [req.params.id, sp.student_profile_id, note || null]);
...
// PATCH /projects/:id — proje düzenle
